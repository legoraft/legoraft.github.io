---
layout: post
title: "Arch Linux install with btrfs and systemd-boot"
updated: 2025-03-22 17:12:00
---

Arch Linux is often referred to as one of the ‘hardest’ Linux distributions to learn, but that isn’t really true. If you can follow instructions thoroughly and are willing to learn things about your Linux system, Arch can be quite simple. This blog post will be the first of two, where I run you through a basic Arch Linux installation and teaching you some of the ins and outs of the Linux system. In the first part, we will set up the basic system with `btrfs` and `systemd-boot` (I’ll explain what those are). In the second part we will set up the graphical environment with a login manager and a Wayland compositor.
...
> [!note]
> Deze post moet twee delen krijgen, eerst de basic system setup en daarna de graphical environment met compositor en display manager.

> [!note]
> Locale kan aangepast worden door bepaalde values in `/etc/locale.conf` aan te passen. Zie de [arch wiki](https://man.archlinux.org/man/locale.conf.5) hiervoor.
